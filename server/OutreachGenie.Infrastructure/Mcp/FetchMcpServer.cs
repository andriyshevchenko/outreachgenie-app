...
/// <summary>
/// MCP server implementation for Fetch web scraping.
/// Provides HTTP request and content extraction capabilities.
/// </summary>
public sealed class FetchMcpServer : IMcpServer
{
    private readonly IMcpTransport transport;
    private readonly ILogger<FetchMcpServer> logger;
    private int requestId;

    /// <summary>
    /// Initializes a new instance of the <see cref="FetchMcpServer"/> class.
    /// </summary>
    /// <param name="transport">The transport for communication.</param>
    /// <param name="logger">Logger for server operations.</param>
    public FetchMcpServer(IMcpTransport transport, ILogger<FetchMcpServer> logger)
    {
    ...
    /// <inheritdoc/>
    public string Name => "Fetch Web Scraping";
    ...
    {
        this.logger.LogInformation("Connecting to Fetch MCP server");
        ...
        this.logger.LogInformation("Fetch MCP server initialized");
    }

    /// <inheritdoc/>
    public async Task DisconnectAsync(CancellationToken cancellationToken = default)
    {
        this.logger.LogInformation("Disconnecting from Fetch MCP server");
    ...
    public async Task<IReadOnlyList<McpTool>> ListToolsAsync(CancellationToken cancellationToken = default)
    {
        ...
        this.logger.LogInformation("Retrieved {ToolCount} tools from Fetch", tools.Count);
        return tools;
    }
    ...
    {
        this.logger.LogInformation("Calling Fetch tool: {ToolName}", name);
